...
## Параметры системы
<details>
  <summary>📊 Показать параметры системы</summary>
  
  <img src="https://github.com/Arseny-Dernovich/Hash_Table/blob/main/%D0%BF%D0%B0%D1%80%D0%B0%D0%BC%D0%B5%D1%82%D1%80%D1%8B_%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D1%8B.png?raw=true" alt="Параметры системы">
</details>
...
Оценивать производительность будем на основе показаний тактов процессора через функцию *rdtsc()*.
...
<details>
<summary>🔧 Показать код измерения производительности</summary>
...
![Github](https://github.com/Arseny-Dernovich/Hash_Table/blob/main/hash_0.png)
...
<details>
<summary>  Код генерации рандомных слов </summary>
...
## Оптимизация rand()
Заменим встроенную функцию rand()  на fast_rand() с помощью интринсиков.
...
![Github](https://github.com/Arseny-Dernovich/Hash_Table/blob/main/hash_1.png)
Как видим , оптимизицая функции rand() помогла оптимизировать программу на примерно 22%.

Продолжим оптимизировать программу дальше , как видно из профилировщика наиболее горячая функция теперь *HashTable_Insert*.
<details>
<summary>  Код ф-ии вставки в хэш-таблицу  </summary>
...
<details>
<summary>  Код оптимизированной ф-ии расчёта хэш-суммы  </summary>
...
; rdi = data (const char*)
; возвращает CRC32 хеш строки, заканчивающейся на \0
; обрабатывает по 4 байта, потом хвост по 1 байту

crc32_hash:
   
    push    rdi             ; сохраняем указатель на начало строки
    call    strlen_32       ; rax = длина
    mov     rsi, rax        ; rsi = len
    pop     rdi             ; rdi = строка снова
...
.loop4:
    ...
    cmp     rsi, 4
    jb      .tail           ; если меньше 4 байт, идём на хвост
    mov     edx, dword [rdi] ; загружаем 4 байта в edx
    crc32   eax, edx
    add     rdi, 4
    sub     rsi, 4
    jmp     .loop4
...
**Стандартное отклонение:** ±2,107,915 тактов


Запустим теперь эту версию программы с двумя оптимизациями под профилировщиком:
![Github](https://github.com/Arseny-Dernovich/Hash_Table/blob/main/hash_2.png)

Как видно  , теперь наиболее горячей функцией является  *strcmp*. Оптимизируем её , переписав на inline assembly , учитывая тот факт , что у меня все слова меньше 32 символов , я могу воспользоваться векторными регистрами YMM , с помощью которых можно сравнивать сразу по 4 буквы за раз.
```c
...
В итоге этих оптимизаций я получил ускорение программы на примерно 29%.   
